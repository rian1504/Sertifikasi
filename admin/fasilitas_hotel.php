...
<div class="row row-cols-1 row-cols-md-1 g-1 m-auto mt-5">
  ...
  ?>
  <div class="col">
    <a href="tambah/tambah_fasilitas_hotel.php?halaman=<?= $halaman ?>" class="btn btn-primary">Tambah Data</a>
  </div>
  <div class="col">
    <table class="table text-center">
      ...
      <tbody>
        <?php
          ...
          $data_fasilitas = mysqli_query($koneksi,"select * from fasilitas_hotel limit $halaman_awal, $batas");
        ...
        <tr>
          ...
          <td><img src="../image/fasilitas_hotel/<?= $row["gambar"] ?>"></td>
          <td>
            <a href="edit/edit_fasilitas_hotel.php?id=<?= $row["id_hotel"] ?>&halaman=<?= $_GET['halaman'] ?>" class="btn btn-warning">Edit</a>
            <a href="hapus/hapus_fasilitas_hotel.php?id=<?= $row["id_hotel"] ?>&halaman=<?= $_GET['halaman'] ?>" class="btn btn-danger" onclick="return confirm('Anda yakin ingin menghapus data?')">Hapus</a>
          </td>
        </tr>
        <?php endwhile; ?>
      </tbody>
